...
from .atomic_context_evaluation import prepare_atomic_context_evaluation
...
from .structure_aware_surface_evaluation import (
...
def _parser() -> argparse.ArgumentParser:
    ...
    continuous_navigation = subcommands.add_parser(
        "prepare-continuous-navigation",
    ...
    learner_navigation = subcommands.add_parser(
        "prepare-learner-navigation",
        help="build the offline SPEC-022 learner navigation grammar experiment",
    )
    learner_navigation.add_argument("--output-dir", required=True, type=Path)
    semantic_depth_review = subcommands.add_parser(
        "prepare-semantic-depth-review",
    ...
    depth_interaction = subcommands.add_parser(
        "prepare-depth-interaction",
    ...
    atomic_context = subcommands.add_parser(
        "prepare-atomic-context",
        help="build the offline SPEC-029 atomic context-transition experiment",
    )
    atomic_context.add_argument("--output-dir", required=True, type=Path)
    learning_surface = subcommands.add_parser(
        "prepare-learning-surface",
    ...
    learning_path = subcommands.add_parser(
        "prepare-learning-path",
        help="build the offline SPEC-032 learning-path navigation experiment",
    )
    learning_path.add_argument("--output-dir", required=True, type=Path)
    revealed_knowledge = subcommands.add_parser(
        "prepare-revealed-knowledge",
    ...
    structure_aware_surface = subcommands.add_parser(
        "prepare-structure-aware-surface",
        help="build the offline SPEC-036 inspectable explanatory surface",
    )
    structure_aware_surface.add_argument("--output-dir", required=True, type=Path)
    visual_semantic_grammar = subcommands.add_parser(
        "prepare-visual-semantic-grammar",
    ...
    semantic_representation_gate = subcommands.add_parser(
        "prepare-semantic-representation-gate",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "prepare-structure-aware-surface":
            report = prepare_structure_aware_surface_evaluation(
                output_dir=args.output_dir
            )
            print(
                f"Wrote {args.output_dir / 'report.json'} "
        ...
        if args.command == "prepare-learning-path":
            report = prepare_learning_path_evaluation(output_dir=args.output_dir)
            print(
                f"Wrote {args.output_dir / 'report.json'} "
                f"(machine integrity {report['machine_integrity_verdict']}; "
                "browser verification pending)"
        ...
        if args.command == "prepare-atomic-context":
            report = prepare_atomic_context_evaluation(output_dir=args.output_dir)
            print(
                f"Wrote {args.output_dir / 'report.json'} "
                f"(machine integrity {report['machine_integrity_verdict']}; "
                "browser verification pending)"
        ...
        if args.command == "prepare-learner-navigation":
            report = prepare_learner_navigation_evaluation(output_dir=args.output_dir)
            print(
                f"Wrote {args.output_dir / 'report.json'} "
